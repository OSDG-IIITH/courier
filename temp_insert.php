		...
		if($change=="1"){
			...
			if($roll1!="0"){  // When the security has entered the roll number of the receiver
				...
				if($num==0){
					...
					$final2=$rst;
					if($num==1){  //No overlap
						$query = "INSERT INTO Courier_Info VALUES ('$cour_id','$type','$name1','$date','$address',0,'$room',0,$match)";
						mysql_query($query) or die ( "not inserted\n");
						...
						if($flag==1){ // There are multiple students with same name but room number matches to one, so courier is send to only that person
							//	header( "Location: security.php?err=2" ) ;
							$query = "INSERT INTO Courier_Info VALUES ('$cour_id','$type','$name1','$date','$address',0,'$room',0,2)";
							mysql_query($query) or die ( "not inserted\n");
						}
			...
			if($num==1){  //No overlap
				$query = "INSERT INTO Courier_Info VALUES ('$cour_id','$type','$name','$date','$address',0,'$room',0,$match)";
				mysql_query($query) or die ( "not inserted\n");
				...
				if($flag==1){ // There are multiple students with same name but room number matches to one, so courier is send to only that person
				//	header( "Location: security.php?err=2" ) ;
					$query = "INSERT INTO Courier_Info VALUES ('$cour_id','$type','$name','$date','$address',0,'$room',0,2)";
					mysql_query($query) or die ( "not inserted\n");
				}
				else if($flag==0){ // Room number doesn't matches with any student, so mail is sent to all of students with that name
